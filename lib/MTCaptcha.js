...
class MTCaptcha {
    ...
    async askGemini(image64) {
        const request = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-robotics-er-1.5-preview:generateContent', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            ...
            body: JSON.stringify({
                contents: [{
                    parts: [
                        { inlineData: { mimeType: 'image/png', data: image64.replace(/[\r\n\s]/g, '').trim() } },
        ...
        if (response.error?.message?.includes('API key not valid')) throw new Error('Gemini API key is not valid');
        if (response.error?.message?.includes('exceeded your quota')) throw new Error('Gemini API key has exceeded its quota');
        if (response.error?.message?.includes('Invalid value at \'contents[0].parts[0].inline_data.data\'')) return { solution: '' };

        const content = response.candidates?.[0]?.content;
        if (!content) console.error('No content received from Gemini:', response);
